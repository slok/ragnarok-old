// Code generated by mockery v1.0.0
package types

import mock "github.com/stretchr/testify/mock"
import nodestatus "github.com/slok/ragnarok/grpc/nodestatus"

import v1 "github.com/slok/ragnarok/api/cluster/v1"

// NodeStateParser is an autogenerated mock type for the NodeStateParser type
type NodeStateParser struct {
	mock.Mock
}

// NodeStateToPB provides a mock function with given fields: state
func (_m *NodeStateParser) NodeStateToPB(state v1.NodeState) (nodestatus.State, error) {
	ret := _m.Called(state)

	var r0 nodestatus.State
	if rf, ok := ret.Get(0).(func(v1.NodeState) nodestatus.State); ok {
		r0 = rf(state)
	} else {
		r0 = ret.Get(0).(nodestatus.State)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(v1.NodeState) error); ok {
		r1 = rf(state)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// PBToNodeState provides a mock function with given fields: state
func (_m *NodeStateParser) PBToNodeState(state nodestatus.State) (v1.NodeState, error) {
	ret := _m.Called(state)

	var r0 v1.NodeState
	if rf, ok := ret.Get(0).(func(nodestatus.State) v1.NodeState); ok {
		r0 = rf(state)
	} else {
		r0 = ret.Get(0).(v1.NodeState)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(nodestatus.State) error); ok {
		r1 = rf(state)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// StrToNodeState provides a mock function with given fields: state
func (_m *NodeStateParser) StrToNodeState(state string) (v1.NodeState, error) {
	ret := _m.Called(state)

	var r0 v1.NodeState
	if rf, ok := ret.Get(0).(func(string) v1.NodeState); ok {
		r0 = rf(state)
	} else {
		r0 = ret.Get(0).(v1.NodeState)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(state)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
