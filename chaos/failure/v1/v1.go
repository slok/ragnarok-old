...
// Metadata has information about the object.
type Metadata struct {
	ID     string // ID is the id of the Failure.
	NodeID string // NodeID is the id of the Node.
}

// Status has all the information of a failure to create an injection
type Status struct {
...
// Spec is the specification that has the information to it can be created and applied.
type Spec struct {
	// Timeout is
	...
	// Status is the status of the Failure.
...
func (f *Failure) GetObjectKind() object.Kind {
	return Kind
}
...
func ReadFailureSpec(data []byte) (Spec, error) {
